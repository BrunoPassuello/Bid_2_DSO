...
class TelaContratoTecnico:
    def tela_inicial_contrato_tecnico(self):
        ...
        with col2:
            st.title("Gerenciamento de Contratos de Técnicos")
        ...
        with st.sidebar:
            ...
            for label, value in menu_items.items():
                if st.button(label, key=f"contrato_t_btn_{value}", use_container_width=True):
                    if value == "retornar":
    ...
    def pega_dados_contrato(self):
        ...
        with st.form(key="contrato_tecnico"):
            ...
            with col1:
                cpf = st.text_input(
                    "CPF do técnico (somente números)", key="contrato_t_cpf", max_chars=11)
                salario = st.number_input("Salário do técnico (R$)",
                                          min_value=0.0,
                                          step=1000.0,
                                          format="%.2f",
                                          key="contrato_t_salario")
            with col2:
                multa_rescisoria = st.number_input("Multa Rescisória (R$)",
                                                   min_value=0.0,
                                                   step=10000.0,
                                                   format="%.2f",
                                                   key="contrato_t_multa")
    ...
    def mostra_contrato(self, contrato):
        ...
        with st.expander(f"Contrato de {contrato.tecnico.nome}", expanded=True):
            col1, col2 = st.columns(2)
            with col1:
    ...
    def seleciona_contrato(self):
        ...
        with st.form(key="seleciona_contrato_tecnico"):
            cpf = st.text_input("CPF do Técnico:",
                                key="contrato_t_select_cpf", max_chars=11)
            submitted = st.form_submit_button(
    ...
    def confirma_demissao(self, contrato):
        ...
        st.write(f"Deseja realmente demitir o técnico {
                 contrato.tecnico.nome}?")
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Confirmar Demissão", key="contrato_t_btn_confirmar_demissao"):
                return True
        with col2:
            if st.button("Cancelar", key="contrato_t_btn_cancelar_demissao"):
                st.session_state.sub_tela = None
                st.rerun()
        return False
